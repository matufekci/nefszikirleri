...
- **İstatistik**: SQL aggregate (günlük, streak, best streak), 7/30 gün ve 6 ay grafik, vakit dağılımı (seher/gündüz/akşam/gece), rozet sistemi, memory-hardened pagination (2000 chunk, `getHistoryPagedDirect` + `getHistoryCountDirect`)
...
- **Hamle 13 (Pagination)**: HistoryDao `getHistoryPagedDirect` + `getHistoryCountDirect` (memory-hardened, 10k+ entries için). O hamlede eklenen `observeHistoryPaged`, `getHistoryByDatePagedDirect`, `getHistoryCountByDateDirect`, `getHistoryCountForZikirDirect` hiç bağlanmadığı için 2026-09-14’te silindi
