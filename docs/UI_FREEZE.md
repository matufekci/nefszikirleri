...
| 2026-09-15 | Compose 2026.09.00 görsel risk analizi (cihaz karşılaştırması rehberi) | Kanıtlı sınır: renkler `AppPalettes`'ten geliyor (`MaterialTheme.colorScheme` kullanımı yok), **15 tipografi stilinin 15'i de `Type.kt`'de tanımlı** ve kodda kullanılan 13 stilin hepsi bu kümede → Material3 varsayılanına sıfır düşüş, yazı boyutu/ağırlığı değişemez; `MaterialTheme.shapes` hiç kullanılmıyor (köşeler açık değer). **Cihazda karşılaştırırken bakılacak tek alan: Material3 bileşen iç davranışları** (Button/Surface/Card/Dialog yüzeyleri, ripple, gölge, animasyon) |
